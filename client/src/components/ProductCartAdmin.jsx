  ...
  return (
    <div className="w-36 p-4 bg-white rounded ">
      ...
      <div className="grid grid-cols-2 gap-3 py-2">
        ...
        <button onClick={()=>setOpenDelete(true)}className="border p-1  text-sm border-red-600 bg-red-100 text-red-800 hover:bg-green-200 rounded">Delete</button>
      </div>
      {
        editOpen && (
      ...
      {
        openDelete && (
          <section className="fixed top-0 bottom-0 left-0 right-0 bg-neutral-600/70 z-50 p-4 flex justify-center items-center ">
            <div className="bg-white p-4 w-full max-w-md rounded-md"> 
              <div className="flex items-center justify-between">
                <h3 className="font-semibold">Permanent Delete</h3>
                <button onClick={()=>setOpenDelete(false)}><IoClose size={20}/></button>
              </div>
